...
def _build_bill(patient_id, form):
    bill = Bill(bill_id="BILL-" + str(uuid.uuid4())[:8].upper(), patient_id=patient_id)
    if form.get("consultation") == "on":
        bill.add_item(ConsultationFee(quantity=int(form.get("consultation_qty", 1))))
    if form.get("ward") == "on":
        bill.add_item(WardFee(days=int(form.get("ward_days", 1))))
    if form.get("lab") == "on":
        bill.add_item(LaboratoryFee(test_name=form.get("lab_test_name", "Test"),
                                    quantity=int(form.get("lab_qty", 1))))
    if form.get("medication") == "on":
        bill.add_item(MedicationFee(medication_name=form.get("med_name", "Medication"),
                                    unit_price=float(form.get("med_unit_price", 0)),
                                    quantity=int(form.get("med_qty", 1))))
    if form.get("surgery") == "on":
        bill.add_item(SurgeryFee(surgery_name=form.get("surgery_name", "Surgery"),
                                 base_cost=float(form.get("surgery_cost", 0))))
    return bill
...
def create():
    ...
    if request.method == "POST":
        ...
        bill = _build_bill(patient_id, request.form)
        if not bill._items:
        ...
        try:
            create_bill(data)
            flash(f"Bill created. Total: ₦{bill.total():,.2f}", "success")
